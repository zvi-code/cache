...
fn bench_rocks_db_all(user_op: &InputOpt, configs_to_run: Vec<BenchPatams>) {
    for bnanch_params in configs_to_run {
        let f = OpenOptions::new()
            .append(true)
            .create(true)
            .open(format!("/tmp/cache_test_{}", bnanch_params.cache_size_mb))
            .expect(&*format!(
                "Unable to open file path /tmp/cache_test_{}",
...
fn bench_with_type<W: Write + std::marker::Sync + std::marker::Send>(
